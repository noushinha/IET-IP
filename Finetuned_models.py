...
base_dir   = "/media/Data/IET IP/Data/" + Dataset + "/5/"
res_dir    = "/media/Data/IET IP/Results/" + Dataset + "/5/"
...
def set_lr(typ="reduce_lr"):
    ...
    lrate = ReduceLROnPlateau(monitor='val_loss', factor=lr_factor,
                              patience=patience, min_lr=min_lr, mode='min', verbose=1)
    if typ == "step_decay":
...
def create_model():
    ...
    model.add(layers.Dense(classes_num, activation='sigmoid'))
...
# if idx == 0:
model.fit(train_data, train_labels_one_hot_coded,
...
for train, vald in kfold.split(train_data, train_labels):
    ...
    history = model.fit(train_data[train], train_labels_one_hot_coded,
                        epochs=fitepoch, batch_size=batchsize, shuffle=shuffle,
...
train_acc = [np.mean([x[i] for x in history_train_acc]) for i in range(fitepoch)]
vald_acc = [np.mean([x[i] for x in history_vald_acc]) for i in range(fitepoch)]
train_loss = [np.mean([x[i] for x in history_train_loss]) for i in range(fitepoch)]
vald_loss = [np.mean([x[i] for x in history_vald_loss]) for i in range(fitepoch)]
train_lr = [np.mean([x[i] for x in history_lrr]) for i in range(fitepoch)]
...
train_labels_one_hot_coded = to_categorical(y=train_labels, num_classes=6)
conv_base2, model = create_model()
model.fit(train_data, train_labels_one_hot_coded, epochs=35, batch_size=batchsize)
...
CF_NonNormalized_filename = os.path.join(eps_dir, Dataset + "_Face_NonNormalized_" + str(fitepoch) + "_Epoch_" + data_type +".eps")
...
CF_Normalized_filename = os.path.join(eps_dir, Dataset + "_Face_Normalized_" + str(fitepoch) + "_Epoch_" + data_type +".eps")
